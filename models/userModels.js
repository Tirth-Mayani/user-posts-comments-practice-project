...
const updateUser = async (user_no, data) => {
    ...
    const result = await pool.query(
        `UPDATE users SET ${setQuery} WHERE user_no = $${keys.length + 1} RETURNING id, user_no, username, display_name, role, email`,
        [...values, user_no]
    );
    return result.rows[0];
}
...
const getUserByEmail = async (email) => {
    const result = await pool.query("SELECT id, username, display_name, email, role, password, user_no FROM users WHERE email = $1", [email]);
    return result.rows[0];
}
